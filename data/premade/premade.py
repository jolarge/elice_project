...
predict = mongo_connect('mongodb://localhost', 27017, 'newsDB', 'predict', username = "root", password = "0000")
premade = mongo_connect('mongodb://localhost', 27017, 'newsDB', 'premade', username = "root", password = "0000")
...
news = pd.DataFrame(predict.find(query,{'_id':False, 'category':False, 'context_url':False}))
...
for i in tqdm(range(len(time))):
    ...
    try:
        ...
        # 긍정,부정,노말 비율
        item_2[t][c][4] += round(labels[i][0], 2)
        item_2[t][c][5] += round(labels[i][1], 2)
        item_2[t][c][6] += round(labels[i][2], 2)

        # 긍정, 부정, 노말 count
        item_2[t][c][1 + labels[i].index(max(labels[i]))] += 1
    except:
        pass
...
js = df.to_json('./' + today + '.json', orient='records', force_ascii=False)
