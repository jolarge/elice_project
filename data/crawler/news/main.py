...
class Crewler(object):
    ...
    @staticmethod
    def make_news_page_url(category_url, start_year, end_year, start_month, end_month, start_day):
        made_urls =[]
        for year in range(start_year, end_year + 1):
            ...
            for month in range(target_start_month, target_end_month + 1):
                if month == target_start_month and year == start_year:
                    for month_day in range(start_day, calendar.monthrange(year, month)[1] + 1):
                        if len(str(month)) == 1:
                            month = "0" + str(month)
    ...
    @staticmethod
    def make_news_page_url_day(category_url, year, month, month_day):
        made_urls = []
        if len(str(month)) == 1:
            month = "0" + str(month)
        if len(str(month_day)) == 1:
        ...
        ## totalpage 계산 네이버 페이지는 page = 10000이 최대
        ## 만약 10000 페이지가 없는 경우 최대 페이지로 redirecting
    ...
    def crawling_everyday(self, category_name):
        # MultiProcess PID
        ...
        today = datetime.now()
        target_urls = self.make_news_page_url_day(url_format, str(today.year), str(today.month), str(today.day-1))
        # print(target_urls)
        print(category_name, "Url are generated")
        print("The crawler starts")

        newsDB = connectDB()
        nomalColliection = newsDB['today']
        ...
        for url in target_urls:
            ...
            for content_url in post_urls:
                ...
                try:
                    ...
                    nomalColliection.insert_one(new)
    ...
    def crawling(self, category_name, start_year, end_year, start_month, end_month, start_day):
        # MultiProcess PID
        print(category_name + "새로운 PID 구동: " + str(os.getpid()))
        total_count = 0
        pass_count = 0
        ...
        target_urls = self.make_news_page_url(url_format, start_year, end_year, start_month, end_month, start_day)
        # print(target_urls)
        print(category_name, "Url are generated")
        print("The crawler starts")

        newsDB = connectDB()
        nomalColliection = newsDB['nosummary2']
        ...
        for url in target_urls:
            ...
            for content_url in post_urls:
                ...
                try:
                    ...
                    new = {'time': time,'category':category_name ,'text_headline':text_headline, 'text_context':text_content,'text_company': text_company, 'context_url': content_url}
    ...
    def start(self):
        ...
        proc = Process(target=self.crawling, args = ('사회',2021, 2021, 11, 11, 27))
        proc.start()

        proc = Process(target=self.crawling, args = ('정치',2021, 2021,11, 11, 27))
        proc.start()

        proc = Process(target=self.crawling, args = ('경제',2021, 2021,11, 11, 27))
        proc.start()

        proc = Process(target=self.crawling, args = ('생활문화',2021, 2021,11, 11, 27))
        proc.start()


if __name__ == "__main__":
    crewler = Crewler()
    # crewler.set_category('정치','경제','사회')
    # crewler.set_date_range(2021, 4, 2021, 11, 24)
    crewler.start()
